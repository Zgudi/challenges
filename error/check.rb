...
#
# 1. The first and last character of the input should be an alphabet between A and Z (both inclusive) and should be in 
# uppercase.
# 
#	2. The input should not contain any space between characters
#
# 3. The input should contain at least 2 numbers between 0 and 9 (both inclusive). The numbers
...
words = ["A123B", "a123B", "A1w3B", "A1 23B", "Au1uu123B"]

words.each do |w|
	if w.match(/(^[A-Z].\d.{0,9}\S*$)/)
		puts "Ok"
	else
		puts "error"
	end
